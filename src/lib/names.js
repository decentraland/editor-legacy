...
/**
 * @summary generate a number from a string. Use the java implementation.
 */
...
/**
 * @summary Picks an element from the left array and combines it with one form right.
 * @description
 *  This function is based on the docker_names implementation in golang.
 *  Added a "seed" that will be respected, and can be a number or string
 *  Will never return "boring_wozniak".
 *  The left array should be an array of adjectives, where as the right array should be
 *  an individuals surname. This results in funny names like "angry bohr" or "prickly murdock".
 * @param {string} identifier - Generate a name based on this identifier
...
function generateName(identifier, left, right) {
  ...
  var second = right[b % right.length];
  var result = first + " " + second;

  /* Steve Wozniak is not boring. This is part of the docker names spec. */
  if (result === "boring wozniak") {
    return generateName('-' + identifier + '-', left, right);
  }
  return result;
}


export default (identifier) => generateName(identifier, left, right);
